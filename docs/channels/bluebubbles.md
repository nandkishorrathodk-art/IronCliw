...
- IronCliw talks to it through its REST API (`GET /api/v1/ping`, `POST /message/text`, `POST /chat/:id/*`).
...
- Webhook authentication is always required. IronCliw rejects BlueBubbles webhook requests unless they include a password/guid that matches `channels.bluebubbles.password` (for example `?password=<password>` or `x-password`), regardless of loopback/proxy topology.
...
- **Typing indicators**: IronCliw sends typing start events; BlueBubbles clears typing automatically on send or timeout (manual stop via DELETE is unreliable).
...
IronCliw may surface _short_ message IDs (e.g., `1`, `2`) to save tokens.
...
- Direct handles: `+15555550123`, `user@example.com`
  - If a direct handle does not have an existing DM chat, IronCliw will create one via `POST /api/v1/chat/new`. This requires the BlueBubbles Private API to be enabled.
