...
Matrix is an open, decentralized messaging protocol. IronCliw connects as a Matrix **user**
...
- On first connection, IronCliw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  IronCliw logs a warning.
...
- On startup, IronCliw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
