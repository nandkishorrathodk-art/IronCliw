---
summary: "Frequently asked questions about Ironcliw setup, configuration, and usage"
read_when:
...
### What's the recommended way to install and set up Ironcliw
...
### How do I install Ironcliw on Linux
...
### How do I install Ironcliw on a VPS
...
### Can I ask Ironcliw to update itself
...
No. You can run Ironcliw with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like Ironcliw.
...
Yes - via **setup-token**. Ironcliw no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so Ironcliw can keep replying while a provider is rate-limited.
...
Yes. Ironcliw fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like Ironcliw. The onboarding wizard can run the OAuth flow for you.
...
Usually no. Ironcliw needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. Ironcliw runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run Ironcliw can I connect it to my MacBook Pro
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but Ironcliw authorization uses numeric IDs only.
...
### Can multiple people use one WhatsApp number with different Ironcliw instances
...
If you run Ironcliw via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
**Ironcliw-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run Ironcliw on a dedicated machine
...
Ironcliw is lightweight. For a basic Gateway + one chat channel:
...
### Can I run Ironcliw in a VM and what are the requirements
...
### What is Ironcliw in one paragraph

Ironcliw is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

Ironcliw is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for Ironcliw
...
### Can Ironcliw help with lead gen outreach ads and blogs for a SaaS
...
Ironcliw draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

Ironcliw is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use Ironcliw when you
...
### Can Ironcliw run tasks on a schedule or continuously in the background
...
Ironcliw memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

Ironcliw also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, Ironcliw auto-selects a provider when it
...
configured and present, Ironcliw
...
### Is all data used with Ironcliw saved locally
...
### How do I completely uninstall Ironcliw
...
`workspace` to the repo root. The Ironcliw repo is just source code; keep the
...
### Can the Ironcliw browser run headless
...
### Can two Ironcliw instances talk to each other local VPS
...
### How does Ironcliw load environment variables
...
### Is there a way to make a team of Ironcliw instances one CEO and many agents
...
### How do I completely reset Ironcliw but keep it installed
...
headers like `# Heading`), Ironcliw skips the heartbeat run to save API calls.
...
No. Ironcliw runs on **your own account**, so if you're in the group, Ironcliw can see it.
...
### Why doesn't Ironcliw reply in a group
...
Ironcliw's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Ironcliw currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
Cooldowns apply to failing profiles (exponential backoff), so Ironcliw can keep responding even when a provider is rate-limited or temporarily failing.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Ironcliw will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: Ironcliw now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
Ironcliw uses provider-prefixed IDs like:
...
### Is it safe to expose Ironcliw to inbound DMs
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Ironcliw only replies to chats it receives or to explicit sends you trigger.
...
Ironcliw blocks **cross-provider** messaging by default. If a tool call is bound
