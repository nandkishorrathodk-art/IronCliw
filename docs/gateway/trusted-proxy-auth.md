...
read_when:
  - Running IronCliw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of IronCliw
...
- You run IronCliw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. IronCliw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. IronCliw extracts the user identity from the configured header
...
- IronCliw can stay on loopback HTTP behind the proxy.
...
If IronCliw itself serves HTTPS directly (no TLS-terminating proxy), set:
