...
read_when:
  - Setting up Ironcliw for the first time
...
<Warning>
Ironcliw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Ironcliw runs your login shell and imports only the missing keys:
