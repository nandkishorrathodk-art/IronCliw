...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Ironcliw runs”).
...
- Legacy `IRONCLIW_GATEWAY_*` env vars are only used by compatibility call paths; probe/status/auth resolution uses `IRONCLIW_GATEWAY_*` only.
