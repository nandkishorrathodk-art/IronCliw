...
- During heartbeat runs, IronCliw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `IronCliw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, IronCliw skips the heartbeat run entirely (no model call).
