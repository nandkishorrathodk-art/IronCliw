...
IronCliw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- IronCliw owns lifecycle when `channels.signal.autoStart=true`.
...
- IronCliw spawns `imsg rpc` as a child process (legacy iMessage integration).
