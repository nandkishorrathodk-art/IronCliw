...
- With Anthropic API-key auth profiles, IronCliw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, IronCliw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
IronCliw exposes dedicated cache-trace diagnostics for embedded agent runs.
