...
This plan defines how IronCliw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- IronCliw core owns ACP control plane concerns
...
IronCliw should not reimplement ACP transport internals in core.
IronCliw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in IronCliw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under IronCliw state dir
...
- ACP event stream is normalized to IronCliw reply chunks
...
For the first production backend (`extensions/acpx`), IronCliw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses IronCliw `sessionKey`)
...
- IronCliw consumes ndjson events from `acpx --format json --json-strict`
