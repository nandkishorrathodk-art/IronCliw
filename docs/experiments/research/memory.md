---
summary: "Research notes: offline memory system for Ironcliw workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.Ironcliw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Ironcliw integration
...
Target: Ironcliw-style workspace (`agents.defaults.workspace`, default `~/.Ironcliw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.Ironcliw/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Ironcliw to see every session.
...
```
~/.Ironcliw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Ironcliw**, but keep a separable core library.

### Why integrate into Ironcliw?

- Ironcliw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `Ironcliw memory recall "…" --k 25 --since 30d`
  - `Ironcliw memory reflect --since 7d`
...
Pragmatic take for `~/.Ironcliw/workspace`:
