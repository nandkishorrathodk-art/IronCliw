...
- `/usage cost` in chats: local cost summary aggregated from Ironcliw session logs.
- CLI: `Ironcliw status --usage` prints a full per-provider breakdown.
- CLI: `Ironcliw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
