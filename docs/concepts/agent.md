...
IronCliw runs a single embedded agent runtime derived from **pi-mono**.
...
Inside `agents.defaults.workspace`, IronCliw expects these user-editable files:
...
On the first turn of a new session, IronCliw injects the contents of these files directly into the agent context.
...
IronCliw reuses pieces of the pi-mono codebase (models/tools), but **session management, discovery, and tool wiring are IronCliw-owned**.
...
- If you omit the provider, IronCliw treats the input as an alias or a model for the **default provider** (only works when there is no `/` in the model ID).
