---
summary: "OAuth in IronCliw: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand IronCliw OAuth end-to-end
...
IronCliw supports “subscription auth” via OAuth for providers that offer it (notably **OpenAI Codex (ChatGPT OAuth)**). For Anthropic subscriptions, use the **setup-token** flow. Anthropic subscription use outside Claude Code has been restricted for some users in the past, so treat it as a user-choice risk and verify current Anthropic policy yourself. OpenAI Codex OAuth is explicitly supported for use in external tools like IronCliw. This page explains:
...
IronCliw also supports **provider plugins** that ship their own OAuth or API‑key
flows. Run them via:
...
- you log in via IronCliw _and_ via Claude Code / Codex CLI → one of them randomly gets “logged out” later

To reduce that, IronCliw treats `auth-profiles.json` as a **token sink**:
...
IronCliw’s interactive login flows are implemented in `@mariozechner/pi-ai` and wired into the wizards/commands.
...
OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including IronCliw workflows.
