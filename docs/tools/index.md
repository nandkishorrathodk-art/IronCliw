...
- If `tools.allow` only references unknown or unloaded plugin tool names, IronCliw logs a warning and ignores the allowlist so core tools stay available.
...
IronCliw tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
Control the dedicated IronCliw-managed browser.
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, IronCliw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, IronCliw clamps `tools.sessions.visibility` to `tree`.
