---
title: Lobster
summary: "Typed workflow runtime for IronCliw with resumable approval gates."
description: Typed workflow runtime for IronCliw — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets IronCliw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: IronCliw runs one Lobster tool call and gets a structured result.
...
IronCliw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In IronCliw tool calls, set `pipeline` to the file path.
...
plugin tools (like `lobster`), IronCliw keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls IronCliw tools that do.
