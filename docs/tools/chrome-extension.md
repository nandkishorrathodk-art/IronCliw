---
summary: "Chrome extension: let IronCliw drive your existing Chrome tab"
read_when:
...
IronCliw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the IronCliw release (npm package) as static files. There is no separate “build” step.
...
IronCliw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want IronCliw to control.
...
- `ON`: attached; IronCliw can drive that tab.
