...
3. Run `ironcliw configure --section web` to store the key in config, or set `BRAVE_API_KEY` in your environment.
...
**Via config:** run `ironcliw configure --section web`. It stores the key under `tools.web.search.apiKey` or `tools.web.search.perplexity.apiKey`, depending on provider.

**Via environment:** set `PERPLEXITY_API_KEY`, `OPENROUTER_API_KEY`, or `BRAVE_API_KEY` in the Gateway process environment. For a gateway install, put it in `~/.ironcliw/.env` (or your service environment). See [Env vars](/help/faq#how-does-ironcliw-load-environment-variables).
