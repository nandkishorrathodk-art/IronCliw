...
read_when:
  - Running IronCliw Gateway in WSL2 while Chrome lives on Windows
...
- IronCliw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/ironcliw/ironcliw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the IronCliw Chrome extension.
...
- IronCliw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, IronCliw is not the problem yet.
...
Fix that before touching IronCliw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point IronCliw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting IronCliw to succeed
...
```bash
ironcliw browser open https://example.com --browser-profile remote
ironcliw browser tabs --browser-profile remote
```

For the extension relay:

```bash
ironcliw browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `ironcliw browser tabs` returns the target
...
3. IronCliw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug IronCliw config or Control UI auth
