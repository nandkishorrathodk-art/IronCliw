...
To reduce top-of-hour load spikes across many gateways, IronCliw applies a
...
If `delivery` is omitted for isolated jobs, IronCliw defaults to `announce`.
...
When a job fails, IronCliw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, IronCliw archives transcript files and purges old deleted archives on the same retention window.
