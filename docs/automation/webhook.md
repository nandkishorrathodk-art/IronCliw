...
read_when:
  ...
  - Wiring external systems into IronCliw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your IronCliw config directory (typically `~/.IronCliw/hooks/transforms`).
...
- `IronCliw webhooks gmail setup` writes `hooks.gmail` config for `IronCliw webhooks gmail run`.
