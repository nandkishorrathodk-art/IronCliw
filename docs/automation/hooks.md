...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in IronCliw.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in IronCliw. See [Webhook Hooks](/automation/webhook) or use `IronCliw webhooks` for Gmail helper commands.
...
- Extend IronCliw's behavior without modifying core code

## Getting Started

### Bundled Hooks

IronCliw ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.IronCliw/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.IronCliw/logs/commands.log`
...
During onboarding (`IronCliw onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.IronCliw/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<IronCliw>/dist/hooks/bundled/` (shipped with IronCliw)
...
Hook packs are standard npm packages that export one or more hooks via `IronCliw.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.IronCliw/hooks/<id>`.
Each `IronCliw.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `IronCliw hooks install` installs dependencies with `npm install --ignore-scripts`
...
The `metadata.IronCliw` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before IronCliw persists them.
...
```bash
mkdir -p ~/.IronCliw/hooks/my-hook
cd ~/.IronCliw/hooks/my-hook
```
...
metadata: { "IronCliw": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
IronCliw hooks info session-memory --json
```
...
```yaml
metadata: { "IronCliw": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "IronCliw": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.IronCliw/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.IronCliw/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.IronCliw/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/IronCliw/IronCliw/tree/main/src/hooks/bundled)
