---
summary: "Use Amazon Bedrock (Converse API) models with IronCliw"
read_when:
  - You want to use Amazon Bedrock models with IronCliw
...
IronCliw can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, IronCliw can automatically discover Bedrock
...
When running IronCliw on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, IronCliw's credential detection currently only checks for environment
...
- IronCliw surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
