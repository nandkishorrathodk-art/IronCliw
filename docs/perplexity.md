...
IronCliw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, IronCliw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `ironcliw configure --section web`. It stores the key in
`~/.ironcliw/ironcliw.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.ironcliw/.env` (or your service environment). See [Env vars](/help/faq#how-does-ironcliw-load-environment-variables).
