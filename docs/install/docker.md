---
summary: "Optional Docker-based setup and onboarding for Ironcliw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Ironcliw on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Ironcliw in Docker)
...
For easier day-to-day Docker management, install `Ironcliwock`:

```bash
mkdir -p ~/.Ironcliwock && curl -sL https://raw.githubusercontent.com/Ironcliw/Ironcliw/main/scripts/shell-helpers/Ironcliwock-helpers.sh -o ~/.Ironcliwock/Ironcliwock-helpers.sh
```

**Add to your shell config (zsh):**

```bash
echo 'source ~/.Ironcliwock/Ironcliwock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `Ironcliwock-start`, `Ironcliwock-stop`, `Ironcliwock-dashboard`, etc. Run `Ironcliwock-help` for all commands.

See [`Ironcliwock` Helper README](https://github.com/Ironcliw/Ironcliw/blob/main/scripts/shell-helpers/README.md) for details.
...
- Custom tools not found: Ironcliw runs commands with `sh -lc` (login shell), which
