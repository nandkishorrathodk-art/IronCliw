...
Deploy IronCliw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/IronCliw/IronCliw)
...
repository configures everything needed to run IronCliw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: IronCliw_STATE_DIR
        value: /data/.IronCliw
      - key: IronCliw_WORKSPACE_DIR
        value: /data/workspace
      - key: IronCliw_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/IronCliw`.
...
If you use the original IronCliw repository, Render will not auto-deploy your IronCliw. To update it, run a manual Blueprint sync from the dashboard.
...
For IronCliw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any IronCliw host.
