...
Deploy Ironcliw on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/Ironcliw-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/Ironcliw` — Control UI

## What you get

- Hosted Ironcliw Gateway + Control UI
...
- `IRONCLIW_STATE_DIR=/data/.Ironcliw` (recommended)
- `IRONCLIW_WORKSPACE_DIR=/data/workspace` (recommended)
- `IRONCLIW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Ironcliw state + workspace so you can migrate to another host without losing config or memory.
