...
IronCliw uses a lobster palette for CLI output.
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, IronCliw may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
IronCliw can surface provider usage/quota when OAuth/API creds are available.
