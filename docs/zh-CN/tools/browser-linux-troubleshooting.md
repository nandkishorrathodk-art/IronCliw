...
summary: 修复 Linux 上 IronCliw 浏览器控制的 Chrome/Brave/Edge/Chromium CDP 启动问题
title: 浏览器故障排除
x-i18n:
...
在 Ubuntu（和许多 Linux 发行版）上，默认的 Chromium 安装是 **snap 包**。Snap 的 AppArmor 限制会干扰 IronCliw 启动和监控浏览器进程的方式。
...
如果你必须使用 snap Chromium，配置 IronCliw 附加到手动启动的浏览器：
...
你正在使用 `chrome` 配置文件（扩展中继）。它期望 IronCliw 浏览器扩展附加到一个活动标签页。
