---
read_when:
  - 制作或验证 Ironcliw macOS 发布版本
  - 更新 Sparkle appcast 或订阅源资源
summary: Ironcliw macOS 发布清单（Sparkle 订阅源、打包、签名）
title: macOS 发布
x-i18n:
...
ditto -c -k --sequesterRsrc --keepParent dist/Ironcliw.app dist/Ironcliw-2026.1.27-beta.1.zip

# 可选：同时构建适合用户使用的样式化 DMG（拖拽到 /Applications）
scripts/create-dmg.sh dist/Ironcliw.app dist/Ironcliw-2026.1.27-beta.1.dmg
...
# 可选：随发布一起提供 dSYM
ditto -c -k --keepParent apps/macos/.build/release/Ironcliw.app.dSYM dist/Ironcliw-2026.1.27-beta.1.dSYM.zip
```
