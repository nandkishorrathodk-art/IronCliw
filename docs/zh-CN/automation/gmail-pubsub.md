---
read_when:
  - 将 Gmail 收件箱触发器接入 IronCliw
  - 为智能体唤醒设置 Pub/Sub 推送
summary: 通过 gogcli 将 Gmail Pub/Sub 推送接入 IronCliw webhooks
title: Gmail PubSub
x-i18n:
...
目标：Gmail watch -> Pub/Sub 推送 -> `gog gmail watch serve` -> IronCliw webhook。
...
- 已启用 IronCliw hooks（参见 [Webhooks](/automation/webhook)）。
