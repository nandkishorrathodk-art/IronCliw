...
- Same path: run the app from a fixed location (for IronCliw, `dist/IronCliw.app`).
...
```bash
sudo tccutil reset Accessibility ai.IronCliw.mac
sudo tccutil reset ScreenCapture ai.IronCliw.mac
...
Workaround: move files into the IronCliw workspace (`~/.IronCliw/workspace`) if you want to avoid per-folder grants.
