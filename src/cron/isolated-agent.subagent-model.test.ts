...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "ironcliw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".ironcliw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<IronCliwConfig> = {},
): IronCliwConfig {
  const base: IronCliwConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<IronCliwConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<IronCliwConfig>,
