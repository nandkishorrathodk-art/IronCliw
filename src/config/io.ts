...
import type { IronCliwConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareIronCliwVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  ironcliw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  ironcliw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: IronCliwConfig,
  pathSegments: string[],
): { changed: boolean; next: IronCliwConfig } {
...
function coerceConfig(value: unknown): IronCliwConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as IronCliwConfig;
}
...
function stampConfigVersion(cfg: IronCliwConfig): IronCliwConfig {
...
function warnIfConfigFromFuture(cfg: IronCliwConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareIronCliwVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer IronCliw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as IronCliwConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): IronCliwConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as IronCliwConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: IronCliwConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as IronCliwConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.IRONCLIW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.IRONCLIW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.IRONCLIW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.IRONCLIW_WATCH_SESSION === "string" &&
        deps.env.IRONCLIW_WATCH_SESSION.trim().length > 0
          ? deps.env.IRONCLIW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.IRONCLIW_WATCH_COMMAND === "string" &&
        deps.env.IRONCLIW_WATCH_COMMAND.trim().length > 0
          ? deps.env.IRONCLIW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `IRONCLIW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: IronCliwConfig | null = null;
let runtimeConfigSourceSnapshot: IronCliwConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.IRONCLIW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.IRONCLIW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): IronCliwConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): IronCliwConfig | null {
...
export function loadConfig(): IronCliwConfig {
...
export async function readBestEffortConfig(): Promise<IronCliwConfig> {
...
export async function writeConfigFile(
  ...
  // edits to ironcliw.json remain visible (no stale snapshot).
}
