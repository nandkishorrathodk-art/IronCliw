...
import type { IronCliwConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: IronCliwConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: IronCliwConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: IronCliwConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = IronCliwSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as IronCliwConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as IronCliwConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as IronCliwConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as IronCliwConfig,
  };
}
