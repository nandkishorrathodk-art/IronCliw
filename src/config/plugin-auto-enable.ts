...
function isWhatsAppConfigured(cfg: IronCliwConfig): boolean {
...
function isGenericChannelConfigured(cfg: IronCliwConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: IronCliwConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: IronCliwConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: IronCliwConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: IronCliwConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: IronCliwConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: IronCliwConfig, pluginId: string): IronCliwConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
