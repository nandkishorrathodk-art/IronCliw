...
function collectConfigEnvVarsByTarget(cfg?: IronCliwConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: IronCliwConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: IronCliwConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: IronCliwConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.IRONCLIW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
