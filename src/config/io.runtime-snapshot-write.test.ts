...
function createSourceConfig(): IronCliwConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): IronCliwConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("ironcliw-config-runtime-source-", async () => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("ironcliw-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".ironcliw", "ironcliw.json");
      ...
      const nextRuntimeConfig: IronCliwConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("ironcliw-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".ironcliw", "ironcliw.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: IronCliwConfig = {
