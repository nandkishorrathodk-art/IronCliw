...
export function buildConfigureCandidates(config: IronCliwConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredIronCliwConfig?: IronCliwConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredIronCliwConfig ?? params.config;
