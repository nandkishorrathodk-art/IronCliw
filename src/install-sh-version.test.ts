...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "ironcliw-install-sh-"));
  const cliPath = path.join(root, "ironcliw");
...
IRONCLIW_BIN="$FAKE_IRONCLIW_BIN"
resolve_ironcliw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_IRONCLIW_BIN: cliPath,
        IRONCLIW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("IronCliw 2026.3.9 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("IronCliw dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("IronCliw dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "ironcliw-install-stdin-"));
