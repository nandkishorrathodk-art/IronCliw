...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked Ironcliw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Ironcliw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked Ironcliw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInIroncliwTmp(
