...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses IRONCLIW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      IRONCLIW_HOME: "/srv/Ironcliw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/Ironcliw-home"), ".Ironcliw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".Ironcliw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("Ironcliw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "Ironcliw-workspace-hardlink-"));
    try {
