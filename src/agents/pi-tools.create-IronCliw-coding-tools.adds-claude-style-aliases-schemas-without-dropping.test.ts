...
describe("createIronCliwCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createIronCliwCodingTools({ senderIsOwner: true }).find(
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createIronCliwTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createIronCliwCodingTools({ messageProvider: "discord" });
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createIronCliwCodingTools({
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createIronCliwReadTool(
      baseRead as unknown as Parameters<typeof createIronCliwReadTool>[0],
    );
