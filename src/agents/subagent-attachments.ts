...
function resolveAttachmentLimits(config: IronCliwConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".ironcliw", "attachments");
  const relDir = path.posix.join(".ironcliw", "attachments", attachmentId);
