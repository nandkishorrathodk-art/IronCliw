...
const GOOGLE_LIVE_SINGLE_KEY = "IRONCLIW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "IRONCLIW_LIVE_ANTHROPIC_KEY",
    listVar: "IRONCLIW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "IRONCLIW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `IRONCLIW_LIVE_${base}_KEY`;
