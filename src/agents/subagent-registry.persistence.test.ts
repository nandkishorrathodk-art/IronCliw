...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["IronCliw_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "IronCliw-subagent-"));
  ...
  it("uses isolated temp state when IronCliw_STATE_DIR is unset in tests", async () => {
