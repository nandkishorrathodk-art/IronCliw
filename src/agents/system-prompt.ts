...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; IronCliw handles all routing internally.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    web_fetch: "Fetch and extract readable content from a URL (pass maxLength=3000 to limit output for large pages)",
    ...
    gateway: "Restart, apply config, or run updates on the running IronCliw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List IronCliw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List IronCliw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside IronCliw.";
  }

  const lines = [
    "You are a personal assistant running inside IronCliw.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control IronCliw's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    "## Context Budget",
    "This gateway may run on a small-context model (e.g. 16k tokens). Treat context space as a scarce resource:",
    "- browser snapshot outputs can be 3-8k tokens each; do NOT take multiple snapshots in one turn unless the page changed.",
    "- web_fetch results can be very long; pass maxLength=3000 (or ask for a short extract) when you only need a summary.",
    "- Avoid reading whole large files when grep/find + targeted read of specific lines suffices.",
    "- If context is growing, summarize earlier results instead of repeating them verbatim in your reply.",
    "",
    ...safetySection,
    ...
    hasGateway && !isMinimal ? "## IronCliw Self-Update" : "",
    ...
    "These user-editable files are loaded by IronCliw and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'IronCliw treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
