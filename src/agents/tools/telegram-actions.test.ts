...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): IronCliwConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as IronCliwConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): IronCliwConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as IronCliwConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: IronCliwConfig, accountId = "media") {
    await handleTelegramAction(
