...
function withDefaultModel(primary: string): IronCliwConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as IronCliwConfig;
}

function withPdfModel(primary: string): IronCliwConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureIronCliwModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
