...
const defaultTools = createIroncliwCodingTools({ senderIsOwner: true });
...
describe("createIroncliwCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: IroncliwConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createIroncliwCodingTools({
    ...
    const anthropicTools = createIroncliwCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: IroncliwConfig = {
      tools: {
        exec: {
    ...
    const allowed = createIroncliwCodingTools({
    ...
    const denied = createIroncliwCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createIroncliwCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
