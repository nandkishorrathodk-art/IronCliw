...
describe("resolveInitialTuiAgentId", () => {
  const cfg: IronCliwConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/ironcliw" },
        { id: "ops", workspace: "/tmp/ironcliw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/ironcliw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run ironcliw devices list");
    expect(state.pairingHint).toContain("ironcliw devices list");
  });
