...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `ironcliw-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  it.each([
    ...
    const result = await withEnvAsync({ IRONCLIW_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("falls back to IRONCLIW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers IRONCLIW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "IRONCLIW_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ironcliw-tui-file-secret-"));
