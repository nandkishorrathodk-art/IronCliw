...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set IRONCLIW_GATEWAY_TOKEN/IRONCLIW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "ironcliw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.IRONCLIW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.IRONCLIW_GATEWAY_PASSWORD);
