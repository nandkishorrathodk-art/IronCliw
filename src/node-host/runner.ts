...
function ensureNodePathEnv(): string {
  ensureIronCliwCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.IronCliw_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.IronCliw_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
