...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "Ironcliw" | "Ironcliw" | "Ironcliw";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["Ironcliw", "Ironcliw", "Ironcliw"] as const;
...
function isIroncliwGatewayLaunchdService(label: string, contents: string): boolean {
...
function isIroncliwGatewayTaskName(name: string): boolean {
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("Ironcliw") || lower.includes("Ironcliw");
}
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "Ironcliw" : "Ironcliw",
        legacy: true,
      });
      continue;
    }
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isIroncliwGatewayTaskName(name)) {
        continue;
      }
