...
export function triggerIroncliwRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.IRONCLIW_SYSTEMD_UNIT,
        process.env.IRONCLIW_PROFILE,
      );
  ...
  const label =
    process.env.IRONCLIW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.IRONCLIW_PROFILE);
