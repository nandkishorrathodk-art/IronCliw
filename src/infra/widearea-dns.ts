...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.IronCliw_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*IronCliw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "IronCliw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "IronCliw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "IronCliw-gw");
  ...
  records.push(`_IronCliw-gw._tcp IN PTR ${instanceLabel}._IronCliw-gw._tcp`);
  records.push(`${instanceLabel}._IronCliw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._IronCliw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; IronCliw-content-hash: ${contentHash}\n${contentBody}`;
}
