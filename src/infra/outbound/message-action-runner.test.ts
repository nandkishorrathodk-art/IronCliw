...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred IronCliw tmp root", async () => {
    const tmpRoot = resolvePreferredIronCliwTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideIronCliw = path.join(os.tmpdir(), "outside-IronCliw", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
