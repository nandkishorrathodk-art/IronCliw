...
function createSinglePluginWithEmptySlotsConfig(): IronCliwConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): IronCliwConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "IronCliw-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-IronCliw-extensions/my-plugin",
      },
      extensionsDir,
    });
