...
function telegramCfg(): IronCliwConfig {
  return { channels: { telegram: { botToken: "tok" } } } as IronCliwConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: IronCliwConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as IronCliwConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as IronCliwConfig;
...
function createSignalAccountOverrideCfg(): IronCliwConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as IronCliwConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as IronCliwConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as IronCliwConfig,
        accountId: undefined,
        params: {
