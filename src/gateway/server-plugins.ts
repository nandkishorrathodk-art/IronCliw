...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "ironcliw.fallbackGatewayContextState",
);
...
export function setFallbackGatewayContext(ctx: GatewayRequestContext): void {
  // Store the gateway context as a fallback. The context object itself uses getters
  // for properties that can change during hot reload (like cron) to stay up-to-date.
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadIronCliwPlugins({
    config: params.cfg,
