...
let lastCreateIronCliwToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/IronCliw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateIronCliwToolsContext?.agentTo,
          agentThreadId: lastCreateIronCliwToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createIronCliwTools: (ctx: Record<string, unknown>) => {
      lastCreateIronCliwToolsContext = ctx;
      return tools;
    },
  };
});
