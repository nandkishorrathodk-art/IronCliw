...
function resolveVoiceTtsConfig(params: { cfg: IronCliwConfig; override?: TtsConfig }): {
  cfg: IronCliwConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredIronCliwTmpDir(), "discord-voice-"));
