...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: IronCliwConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: IronCliwConfig) => { changed: boolean; next: IronCliwConfig };
  defaultModel: string;
  overrideConfig: IronCliwConfig;
  alreadyDefaultConfig: IronCliwConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: IronCliwConfig) => config,
      applyDefaultConfig: (config: IronCliwConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: IronCliwConfig = { agents: { defaults: {} } };
