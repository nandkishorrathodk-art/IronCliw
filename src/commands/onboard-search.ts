...
function rawKeyValue(config: IronCliwConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: IronCliwConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: IronCliwConfig, provider: SearchProvider): IronCliwConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: IronCliwConfig, result: IronCliwConfig): IronCliwConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with ironcliw configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — IronCliw will store a reference instead of the API key.",
