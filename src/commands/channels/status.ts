...
import { type IronCliwConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export function formatGatewayChannelsStatusLines(payload: Record<string, unknown>): string[] {
  ...
  if (issues.length > 0) {
    ...
    lines.push(`- Run: ${formatCliCommand("ironcliw doctor")}`);
    lines.push("");
  }
  lines.push(
...
export async function formatConfigChannelsStatusLines(
  ...
  opts?: { sourceConfig?: IronCliwConfig },
): Promise<string[]> {
  const lines: string[] = [];
