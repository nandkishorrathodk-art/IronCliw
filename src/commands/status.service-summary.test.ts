...
describe("readServiceStatusSummary", () => {
  it("marks IronCliw-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["ironcliw", "gateway", "run"] })),
    ...
    expect(summary.managedByIronCliw).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByIronCliw).toBe(false);
