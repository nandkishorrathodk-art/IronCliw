...
export function normalizeCompatibilityConfigValues(cfg: IroncliwConfig): {
  config: IroncliwConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: IroncliwConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as IroncliwConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as IroncliwConfig["browser"],
    };
    changes.push(
