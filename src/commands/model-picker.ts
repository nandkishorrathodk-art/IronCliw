...
type PromptDefaultModelResult = { model?: string; config?: IronCliwConfig };
...
function resolveConfiguredModelRaw(cfg: IronCliwConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: IronCliwConfig): string[] {
...
export function applyPrimaryModel(cfg: IronCliwConfig, model: string): IronCliwConfig {
...
export function applyModelAllowlist(cfg: IronCliwConfig, models: string[]): IronCliwConfig {
  const defaults = cfg.agents?.defaults;
