...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByIronCliw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByIronCliw ? "installed · " : "";
  ...
  runtime.log(theme.heading("IronCliw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`ironcliw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("ironcliw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("ironcliw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("ironcliw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("ironcliw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.ironcliw.ai/faq");
  runtime.log("Troubleshooting: https://docs.ironcliw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("ironcliw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("ironcliw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("ironcliw status --deep")}`);
  } else {
