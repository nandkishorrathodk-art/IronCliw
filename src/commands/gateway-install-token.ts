...
import { readConfigFileSnapshot, writeConfigFile, type IronCliwConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("ironcliw config set gateway.auth.mode token")} or ${formatCliCommand("ironcliw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
