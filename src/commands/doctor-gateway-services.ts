...
function resolveGatewayAuthToken(cfg: IroncliwConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.IRONCLIW_GATEWAY_TOKEN ?? env.IRONCLIW_GATEWAY_TOKEN;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy gateway services (Ironcliw/Ironcliw) now?",
      initialValue: true,
    });
    if (shouldRemove) {
      const removed: string[] = [];
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing Ironcliw gateway next.");
      }
    }
  }
