...
function resolveAgentName(cfg: IronCliwConfig, agentId: string) {
...
function resolveAgentModel(cfg: IronCliwConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: IronCliwConfig): AgentSummary[] {
