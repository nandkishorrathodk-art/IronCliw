...
export async function backupCreateCommand(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No IronCliw config file was found to back up."
        : "No local IronCliw state was found to back up.",
    );
  }
  ...
  if (!opts.dryRun) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ironcliw-backup-"));
