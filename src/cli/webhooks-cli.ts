...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.Ironcliw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Ironcliw hooks")
    ...
    .option("--hook-url <url>", "Ironcliw hook URL")
    .option("--hook-token <token>", "Ironcliw hook token")
