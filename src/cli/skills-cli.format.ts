...
function appendironcliwHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
  return `${output}\n\nTip: use \`npx ironcliw\` to search, install, and sync skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("Ironcliw skills list")}\` to see all skills.`
      : "No skills found.";
    return appendironcliwHint(message, opts.json);
  }
  ...
  return appendironcliwHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendironcliwHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("Ironcliw skills list")}\` to see available skills.`,
      opts.json,
    );
  }
