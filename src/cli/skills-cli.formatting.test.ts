...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "ironcliw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "ironcliw-bundled-skills-test-"));
    process.env.IRONCLIW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
