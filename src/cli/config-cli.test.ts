...
import type { ConfigFileSnapshot, IronCliwConfig } from "../config/types.js";
...
const mockWriteConfigFile = vi.fn<
  (cfg: IronCliwConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: IronCliwConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: IronCliwConfig, config: IronCliwConfig) {
...
function withRuntimeDefaults(resolved: IronCliwConfig): IronCliwConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: IronCliwConfig = {
