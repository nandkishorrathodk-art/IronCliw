...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("ironcliw-daemon-install-int-");
    configPath = path.join(tempHome, "ironcliw.json");
    process.env.HOME = tempHome;
    process.env.IRONCLIW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.IRONCLIW_GATEWAY_TOKEN).toBeUndefined();
  });
});
