...
describe("update-cli", () => {
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveIronCliwPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveIronCliwPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("IronCliw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as IronCliwConfig,
        });
      },
