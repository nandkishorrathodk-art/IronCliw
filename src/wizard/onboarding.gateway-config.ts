...
import type { GatewayBindMode, GatewayTailscaleMode, IronCliwConfig } from "../config/config.js";
...
export async function configureGatewayForOnboarding(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.IRONCLIW_GATEWAY_TOKEN)) ||
        randomToken();
