...
type HomeEnvSnapshot = {
  ...
  IronCliw_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    IronCliw_AGENT_DIR: process.env.IronCliw_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.IronCliw_AGENT_DIR = path.join(home, ".IronCliw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".IronCliw", "agent");
