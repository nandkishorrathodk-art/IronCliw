...
export function isCommandEnabled(cfg: IroncliwConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: IroncliwConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as IroncliwConfig),
...
export function getCommandDetection(_cfg?: IroncliwConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: IroncliwConfig) {
