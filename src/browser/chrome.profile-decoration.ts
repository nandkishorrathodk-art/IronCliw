...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".Ironcliw-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + ironcliw-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateIroncliwProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_IRONCLIW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_IRONCLIW_BROWSER_COLOR).toUpperCase();
