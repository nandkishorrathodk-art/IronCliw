...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredIronCliwTmpDir: vi.fn(() => "/tmp/IronCliw"),
}));
vi.mock("../infra/tmp-IronCliw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredIronCliwTmpDir.mockReturnValue("/tmp/IronCliw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "IronCliw-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".IronCliw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredIronCliwTmpDir.mockReturnValue("/tmp/IronCliw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "IronCliw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "IronCliw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredIronCliwTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "IronCliw-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "IronCliw-preferred", "downloads")}${path.sep}`),
    );
  });
