...
final class RemindersService: RemindersServicing {
    func list(params: IroncliwRemindersListParams) async throws -> IroncliwRemindersListPayload {
        ...
        let payload: [IroncliwReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return IroncliwReminderPayload(
        ...
        return IroncliwRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: IroncliwRemindersAddParams) async throws -> IroncliwRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = IroncliwReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return IroncliwRemindersAddPayload(reminder: payload)
    }
