...
final class MotionService: MotionServicing {
    func activities(params: IronCliwMotionActivityParams) async throws -> IronCliwMotionActivityPayload {
        ...
        let mapped: [IronCliwMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return IronCliwMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: IronCliwPedometerParams) async throws -> IronCliwPedometerPayload {
        ...
        let payload: IronCliwPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = IronCliwPedometerPayload(
