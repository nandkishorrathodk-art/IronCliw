...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> IroncliwDeviceStatusPayload {
        ...
        return IroncliwDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> IroncliwDeviceInfoPayload {
        ...
        return IroncliwDeviceInfoPayload(
    ...
    private func batteryStatus() -> IroncliwBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: IroncliwBatteryState = switch device.batteryState {
        ...
        return IroncliwBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> IroncliwThermalStatusPayload {
        let state: IroncliwThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return IroncliwThermalStatusPayload(state: state)
    }

    private func storageStatus() -> IroncliwStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return IroncliwStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
