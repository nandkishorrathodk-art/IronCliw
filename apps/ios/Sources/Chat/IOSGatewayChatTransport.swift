...
struct IOSGatewayChatTransport: IronCliwChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> IronCliwChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(IronCliwChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> IronCliwChatHistoryPayload {
        ...
        return try JSONDecoder().decode(IronCliwChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [IronCliwChatAttachmentPayload]) async throws -> IronCliwChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [IronCliwChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(IronCliwChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(IronCliwGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: IronCliwGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: IronCliwChatEventPayload.self)
                        {
