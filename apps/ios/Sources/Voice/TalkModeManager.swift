...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__IronCliw_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<IronCliwTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.IronCliw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = IronCliwTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> IronCliwTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return IronCliwTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> IronCliwTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> IronCliwTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return IronCliwTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> IronCliwTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = IronCliwTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: IronCliwTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: IronCliwAgentEventPayload.self
            ) else {
                continue
            }
