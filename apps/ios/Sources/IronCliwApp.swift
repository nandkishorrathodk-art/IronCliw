...
enum WatchPromptNotificationBridge {
    ...
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: IronCliwWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> IronCliwWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return IronCliwWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "IronCliw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [IronCliwWatchAction]) -> [UNNotificationAction] {
...
@main
struct IronCliwApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(IronCliwAppDelegate.self) private var appDelegate
...
extension IronCliwApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("IronCliw: installing uncaught exception handler")
