...
private const val NOTIFICATION_CHANNEL_BASE_ID = "IronCliw.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "IronCliw Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "IronCliw Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "IronCliw Active")
      }
