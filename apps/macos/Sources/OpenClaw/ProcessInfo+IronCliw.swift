...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.ironcliw.mac`) even if the shipped
    ...
    {
        if environment["IRONCLIW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "ironcliw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "ironcliw.nixMode") { return true }
