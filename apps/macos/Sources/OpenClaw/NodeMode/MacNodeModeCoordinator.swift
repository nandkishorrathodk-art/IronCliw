...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.ironcliw", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = IronCliwConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: IronCliwNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [IronCliwCapability.canvas.rawValue, IronCliwCapability.screen.rawValue]
        if IronCliwConfigFile.browserControlEnabled() {
            caps.append(IronCliwCapability.browser.rawValue)
        }
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(IronCliwCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if IronCliwLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(IronCliwCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            IronCliwSystemCommand.which.rawValue,
            IronCliwSystemCommand.run.rawValue,
            IronCliwSystemCommand.execApprovalsGet.rawValue,
            IronCliwSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(IronCliwCapability.browser.rawValue) {
            commands.append(IronCliwBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(IronCliwCapability.camera.rawValue) {
        ...
        if capsSet.contains(IronCliwCapability.location.rawValue) {
