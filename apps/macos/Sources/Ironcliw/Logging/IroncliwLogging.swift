...
enum IroncliwLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = IroncliwOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = IroncliwFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        IroncliwLogging.bootstrapIfNeeded()
        let label = IroncliwLogging.makeLabel(subsystem: subsystem, category: category)
...
struct IroncliwOSLogHandler: AppLogLevelBackedHandler {
...
struct IroncliwFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = IroncliwLogging.parseLabel(self.label)
