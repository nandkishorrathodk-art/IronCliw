...
enum DebugActions {
    private static let verboseDefaultsKey = "IronCliw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "IronCliw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = IronCliwPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "IronCliw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = IronCliwPaths.configURL
        guard
