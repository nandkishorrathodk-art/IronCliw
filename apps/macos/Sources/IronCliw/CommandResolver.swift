...
enum CommandResolver {
    private static let projectRootDefaultsKey = "IronCliw.gatewayProjectRootPath"
    private static let helperName = "IronCliw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let IronCliwEntry = root.appendingPathComponent("IronCliw.mjs").path
        if FileManager().isReadableFile(atPath: IronCliwEntry) { return IronCliwEntry }
        let binEntry = root.appendingPathComponent("bin/IronCliw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/IronCliw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let IronCliwPaths = self.IronCliwManagedPaths(home: home)
        if !IronCliwPaths.isEmpty {
            extras.insert(contentsOf: IronCliwPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + IronCliwPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func IronCliwManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".IronCliw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func IronCliwExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectIronCliwExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("IronCliw.mjs").path,
            root.appendingPathComponent("bin/IronCliw.js").path,
        ]
    ...
    static func hasAnyIronCliwInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.IronCliwExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func IronCliwNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let IronCliwPath = self.projectIronCliwExecutable(projectRoot: root) {
            return [IronCliwPath, subcommand] + extraArgs
        }
        if let IronCliwPath = self.IronCliwExecutable(searchPaths: searchPaths) {
            return [IronCliwPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "IronCliw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            IronCliw entrypoint missing (looked for dist/index.js or IronCliw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real IronCliw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/IronCliw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v IronCliw >/dev/null 2>&1; then
          CLI="$(command -v IronCliw)"
          IronCliw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/IronCliw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/IronCliw.mjs"
            node "$PRJ/IronCliw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/IronCliw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/IronCliw.js"
            node "$PRJ/bin/IronCliw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent IronCliw"
          pnpm --silent IronCliw \(quotedArgs);
        else
          echo "IronCliw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? IronCliwConfigFile.loadDict()
