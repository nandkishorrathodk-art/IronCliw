...
private let webChatSwiftLogger = Logger(subsystem: "ai.IronCliw", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: IronCliwChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> IronCliwChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(IronCliwChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> IronCliwChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                IronCliwGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    IronCliwAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<IronCliwChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any IronCliwChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = IronCliwChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: IronCliwChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<IronCliwChatView>) -> NSViewController
    {
