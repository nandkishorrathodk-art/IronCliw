...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["IronCliw_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/IronCliw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("IronCliw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("IronCliw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/IronCliw/ (rolling or stdout), or nil if none exist.
