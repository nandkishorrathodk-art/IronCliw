...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: IronCliwProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: IronCliwProtocol.AnyCodable] = [
            "presence": .init([IronCliwProtocol.AnyCodable(entry)]),
        ]
        let payload = IronCliwProtocol.AnyCodable(payloadMap)
