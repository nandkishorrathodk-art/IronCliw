...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers ironcliw binary`() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let ironcliwPath = tmp.appendingPathComponent("node_modules/.bin/ironcliw")
        try makeExecutableForTests(at: ironcliwPath)

        let cmd = CommandResolver.ironcliwCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([ironcliwPath.path, "gateway"]))
    }
    ...
    @Test func `prefers ironcliw binary over pnpm`() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let ironcliwPath = binDir.appendingPathComponent("ironcliw")
        ...
        #expect(cmd.prefix(2).elementsEqual([ironcliwPath.path, "rpc"]))
    }

    @Test func `uses ironcliw binary without node runtime`() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "ironcliw", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "ironcliw", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("ironcliw@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/ironcliw", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "ironcliw@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/ironcliw'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("ironcliw"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == ironcliwPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
