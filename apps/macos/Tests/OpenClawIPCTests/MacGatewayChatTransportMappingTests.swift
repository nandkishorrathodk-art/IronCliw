...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: IronCliwProtocol.AnyCodable(["ok": IronCliwProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: IronCliwProtocol.AnyCodable(["ok": IronCliwProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = IronCliwProtocol.AnyCodable([
            "runId": IronCliwProtocol.AnyCodable("run-1"),
            "sessionKey": IronCliwProtocol.AnyCodable("main"),
            "state": IronCliwProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: IronCliwProtocol.AnyCodable(["a": IronCliwProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
