...
import Foundation
import Testing
@testable import IroncliwChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> IroncliwChatHistoryPayload
{
    IroncliwChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> IroncliwChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [IroncliwChatHistoryPayload],
    sessionsResponses: [IroncliwChatSessionsListResponse] = []) async -> (TestChatTransport, IroncliwChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { IroncliwChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: IroncliwChatViewModel,
...
private func sendUserMessage(_ vm: IroncliwChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, IroncliwChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [IroncliwChatHistoryPayload]
    private let sessionsResponses: [IroncliwChatSessionsListResponse]

    private let stream: AsyncStream<IroncliwChatTransportEvent>
    private let continuation: AsyncStream<IroncliwChatTransportEvent>.Continuation

    init(
        historyResponses: [IroncliwChatHistoryPayload],
        sessionsResponses: [IroncliwChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<IroncliwChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> IroncliwChatHistoryPayload {
        ...
        return self.historyResponses.last ?? IroncliwChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [IroncliwChatAttachmentPayload]) async throws -> IroncliwChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return IroncliwChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> IroncliwChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? IroncliwChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: IroncliwChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = IroncliwChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = IroncliwChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { IroncliwChatViewModel(sessionKey: "main", transport: transport) }
