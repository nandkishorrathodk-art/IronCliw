...
@MainActor
public struct IronCliwChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: IronCliwChatViewModel
    ...
    public init(
        viewModel: IronCliwChatViewModel,
    ...
    private var visibleMessages: [IronCliwChatMessage] {
        let base: [IronCliwChatMessage]
    ...
    private func mergeToolResults(in messages: [IronCliwChatMessage]) -> [IronCliwChatMessage] {
        ...
        for message in messages {
            ...
            let merged = IronCliwChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: IronCliwChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: IronCliwChatMessage) -> Bool {
    ...
    private func primaryText(in message: IronCliwChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: IronCliwChatMessage) -> Bool {
    ...
    private func toolCalls(in message: IronCliwChatMessage) -> [IronCliwChatMessageContent] {
    ...
    private func inlineToolResults(in message: IronCliwChatMessage) -> [IronCliwChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: IronCliwChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: IronCliwChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(IronCliwChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
