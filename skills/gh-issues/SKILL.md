...
If still empty, check `/data/.clawdbot/IronCliw.json`:

```
cat /data/.clawdbot/IronCliw.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
GH_TOKEN=$(cat ~/.IronCliw/IronCliw.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/IronCliw.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/IronCliw.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")
