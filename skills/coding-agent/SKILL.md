...
**⚠️ CRITICAL: Never review PRs in IronCliw's own project folder!**
...
For long-running background tasks, append a wake trigger to your prompt so IronCliw gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):
