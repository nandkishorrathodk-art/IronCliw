---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for IronCliw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, IronCliw cron scheduling for periodic checks, or version status checks on a machine running IronCliw (laptop, workstation, Pi, VPS).
---

# IronCliw Host Hardening

## Overview

Assess and harden the host running IronCliw, then align it to a user-defined risk tolerance without breaking access. Use IronCliw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim IronCliw changes the host firewall, SSH, or OS updates; it does not.
...
5. IronCliw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if IronCliw can access sensitive data.
...
- Credential hygiene notes (location of IronCliw creds, prefer disk encryption)
...
- IronCliw security audit (re-run)
...
Follow the durable-memory prompt format used by IronCliw compaction:
