  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __IRONCLIW_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__IRONCLIW_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__IRONCLIW_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /ironcliw/ ");

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/ironcliw"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/ironcliw/chat",
    });

    const { loadSettings } = await import("./storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/ironcliw"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/ironcliw",
    ...
    expect(JSON.parse(localStorage.getItem("ironcliw.control.settings.v1") ?? "{}")).toEqual({
      gatewayUrl: "wss://gateway.example:8443/ironcliw",
      sessionKey: "agent",
