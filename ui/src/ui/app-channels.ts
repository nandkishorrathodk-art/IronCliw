...
export async function handleWhatsAppStart(host: IronCliwApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: IronCliwApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: IronCliwApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: IronCliwApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: IronCliwApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: IronCliwApp): string {
...
function resolveGatewayHttpAuthHeader(host: IronCliwApp): string | null {
...
function buildGatewayHttpHeaders(host: IronCliwApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: IronCliwApp) {
...
export function handleNostrProfileToggleAdvanced(host: IronCliwApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: IronCliwApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: IronCliwApp) {
  const state = host.nostrProfileFormState;
