...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "ironlog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
ironlog - IronCliw Logging Utility

USAGE:
    ironlog [OPTIONS]

DESCRIPTION:
    View IronCliw logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    IronCliw logs flow through the macOS unified log (subsystem: ai.IronCliw).
...
QUICK START:
    ironlog -n 100             Show last 100 lines from all components
    ironlog -f                 Follow logs in real-time
    ironlog -e                 Show only errors
    ironlog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    ironlog                   Show last 50 lines from past 5 minutes (default)
    ironlog -f                Stream logs continuously
    ironlog -n 100            Show last 100 lines
    ironlog -e                Show only recent errors
    ironlog -l 30m -n 200     Show last 200 lines from past 30 minutes
    ironlog -c ServerManager  Show recent ServerManager logs
    ironlog -s "fail"         Search for "fail" in recent logs
    ironlog --server -e       Show recent server errors
    ironlog -f -d             Stream debug logs continuously
