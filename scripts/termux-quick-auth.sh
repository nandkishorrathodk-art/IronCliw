...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/Ironcliw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Ironcliw Re-Auth" -c "After getting key, run: ssh $SERVER '~/Ironcliw/scripts/mobile-reauth.sh'" --id Ironcliw-auth
        ;;
    *)
