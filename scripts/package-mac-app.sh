...
# Build and bundle IronCliw into a minimal .app we can open.
# Outputs to dist/IronCliw.app
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/IronCliw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :IronCliwBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :IronCliwGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/IronCliw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/IronCliw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/IronCliw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/IronCliw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/IronCliw/Resources/IronCliw.icns" "$APP_ROOT/Contents/Resources/IronCliw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/IronCliw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "⏹  Stopping any running IronCliw"
killall -q IronCliw 2>/dev/null || true
