...
type DriverMode = "token" | "webhook" | "Ironcliw";
...
function resolveStateDir(): string {
  const override = process.env.IRONCLIW_STATE_DIR?.trim() || process.env.IRONCLIW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.IRONCLIW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".Ironcliw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver Ironcliw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Ironcliw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|Ironcliw> Driver transport mode (default: token)\n" +
    ...
    "  --Ironcliw-bin <path>        Ironcliw CLI binary for driver=Ironcliw (default: Ironcliw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  IRONCLIW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  IRONCLIW_DISCORD_SMOKE_DRIVER\n" +
    "  IRONCLIW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  IRONCLIW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  IRONCLIW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  IRONCLIW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  IRONCLIW_DISCORD_SMOKE_AGENT\n" +
    "  IRONCLIW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  IRONCLIW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  IRONCLIW_DISCORD_SMOKE_POLL_MS\n" +
    "  IRONCLIW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  IRONCLIW_DISCORD_SMOKE_IRONCLIW_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.IRONCLIW_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.IRONCLIW_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.IRONCLIW_DISCORD_SMOKE_DRIVER ||
    process.env.IRONCLIW_DISCORD_SMOKE_DRIVER ||
    "token";
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.IRONCLIW_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.IRONCLIW_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.IRONCLIW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.IRONCLIW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.IRONCLIW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.IRONCLIW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.IRONCLIW_DISCORD_SMOKE_AGENT ||
    process.env.IRONCLIW_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.IRONCLIW_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.IRONCLIW_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.IRONCLIW_DISCORD_SMOKE_INSTRUCTION ||
    process.env.IRONCLIW_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.IRONCLIW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.IRONCLIW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.IRONCLIW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const IroncliwBin =
    resolveArg("--Ironcliw-bin") || process.env.IRONCLIW_DISCORD_SMOKE_IRONCLIW_BIN || "Ironcliw";
...
async function IroncliwCliJson<T>(params: { IroncliwBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.IroncliwBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`Ironcliw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithIroncliw(params: {
  IroncliwBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await IroncliwCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    IroncliwBin: params.IroncliwBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "Ironcliw") {
    return await readMessagesWithIroncliw({
      IroncliwBin: params.args.IroncliwBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `Ironcliw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await IroncliwCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("Ironcliw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "Ironcliw"
            ? await readMessagesWithIroncliw({
                IroncliwBin: args.IroncliwBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Ironcliw.`,
        diagnostics: {
          bindingCandidates: [
            {
