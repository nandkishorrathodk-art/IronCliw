...
import type { ApiContributor, Entry, MapConfig, User } from "./update-ironcliw-tributors.types.js";

const REPO = "IronCliw/IronCliw";
const PER_LINE = 10;

const mapPath = resolve("scripts/ironcliw-tributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing ironcliw-tributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README ironcliw-tributors: ${entries.length} entries`);
