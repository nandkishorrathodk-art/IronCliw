#!/usr/bin/env bash
# Reset IronCliw like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product IronCliw"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/IronCliw.app" ]]; then
    APP_BUNDLE="/Applications/IronCliw.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/IronCliw.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/IronCliw.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/IronCliw.app missing Sparkle after packaging"
    fi
    return 0
  fi
