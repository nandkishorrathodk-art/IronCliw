...
IMAGE_NAME="IronCliw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="IronCliw-net-e2e-$$"
GW_NAME="IronCliw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "IronCliw_GATEWAY_TOKEN=$TOKEN" \
  -e "IronCliw_SKIP_CHANNELS=1" \
  -e "IronCliw_SKIP_GMAIL_WATCHER=1" \
  -e "IronCliw_SKIP_CRON=1" \
  -e "IronCliw_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
