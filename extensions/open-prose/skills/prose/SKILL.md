...
metadata: { "IronCliw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == IronCliw `sessions_spawn`
- **File I/O** == IronCliw `read`/`write`
- **Remote fetch** == IronCliw `web_fetch` (or `exec` with curl when POST is required)
