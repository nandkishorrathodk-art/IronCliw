import type { AnyAgentTool, IroncliwPluginApi } from "Ironcliw/plugin-sdk";
...
export function createToolFactoryHarness(cfg: IroncliwPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<IroncliwPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
