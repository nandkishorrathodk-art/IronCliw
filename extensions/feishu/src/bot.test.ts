...
async function dispatchMessage(params: { cfg: IroncliwConfig; event: FeishuMessageEvent }) {
...
describe("resolveBroadcastAgents", () => {
  it("returns agent list when broadcast config has the peerId", () => {
    const cfg = { broadcast: { oc_group123: ["susan", "main"] } } as unknown as IroncliwConfig;
  ...
  it("returns null when peerId not in broadcast", () => {
    const cfg = { broadcast: { oc_other: ["susan"] } } as unknown as IroncliwConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });

  it("returns null when agent list is empty", () => {
    const cfg = { broadcast: { oc_group123: [] } } as unknown as IroncliwConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });
});
