...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__ironcliw_channel_id";
...
type InteractionCallbackConfig = Pick<IronCliwConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "ironcliw-mattermost-interactions").update(botToken).digest("hex");
}
