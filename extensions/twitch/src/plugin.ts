/**
 * Twitch channel plugin for Ironcliw.
 *
...
import { buildChannelConfigSchema } from "Ironcliw/plugin-sdk";
 ...
 * for Ironcliw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: IroncliwConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: IroncliwConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: IroncliwConfig): boolean => {
