...
import type { IroncliwPluginApi, IroncliwPluginService } from "Ironcliw/plugin-sdk";
...
function normalizeDenyList(cfg: IroncliwPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: IroncliwPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.allowCommands ?? [])]);
}

function patchConfigNodeLists(
  cfg: IroncliwPluginApi["config"],
  next: { allowCommands: string[]; denyCommands: string[] },
): IroncliwPluginApi["config"] {
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
...
export default function register(api: IroncliwPluginApi) {
  ...
  const timerService: IroncliwPluginService = {
