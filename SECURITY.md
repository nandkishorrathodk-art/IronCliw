...
If you believe you've found a security issue in IronCliw, please report it privately.
...
- Tested version details (IronCliw version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to IronCliw's documented trust boundaries.
- For exposed-secret reports: proof the credential is IronCliw-owned (or grants access to IronCliw-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at IronCliw. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

IronCliw is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
IronCliw does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- IronCliw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need IronCliw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in IronCliw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of IronCliw's trusted computing base for a gateway.
...
- Exposed secrets that are third-party/user-controlled credentials (not IronCliw-owned and not granting access to IronCliw-operated infrastructure/services) without demonstrated IronCliw impact
...
IronCliw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
IronCliw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the IronCliw process.
...
- Sandbox media validation allows absolute temp paths only under the IronCliw-managed temp root.
...
IronCliw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.
...
IronCliw requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running IronCliw in Docker:
