...
- Security/Logging utility hardening: remove `eval`-based command execution from `scripts/ironlog.sh`, switch to argv-safe command construction, and escape predicate literals for user-supplied search/category filters to block local command/predicate injection paths.
...
- Scripts/Security: validate GitHub logins and avoid shell invocation in `scripts/update-ironcliw-tributors.ts` to prevent command injection via malicious commit records. Thanks @scanleale.
