...
- Browser/Gateway hardening: preserve env credentials for `IRONCLIW_GATEWAY_URL` / `IRONCLIW_GATEWAY_URL` while treating explicit `--url` as override-only auth, and make container browser hardening flags optional with safer defaults for Docker/LXC stability. (#31504) Thanks @vincentkoc.
...
- macOS/PeekabooBridge: add compatibility socket symlinks for legacy `Ironcliw`, `clawdis`, and `Ironcliw` Application Support socket paths so pre-rename clients can still connect. (#6033) Thanks @lumpinif and @vincentkoc.
...
- Docs/Docker images: clarify the official GHCR image source and tag guidance (`main`, `latest`, `<version>`), and document that `IRONCLIW_IMAGE` skips local image builds but still uses the repo-local compose/setup flow. (#27214, #31180) Fixes #15655. Thanks @ipl31.
...
- Install/Discord Voice: make `@discordjs/opus` an optional dependency so `Ironcliw` install/update no longer hard-fails when native Opus builds fail, while keeping `opusscript` as the runtime fallback decoder for Discord voice flows. (#23737, #23733, #23703) Thanks @jeadland, @Sheetaa, and @Breakyman.
...
- Security/Sandbox: remove default `--no-sandbox` for the browser container entrypoint, add explicit opt-in via `IRONCLIW_BROWSER_NO_SANDBOX` / `IRONCLIW_BROWSER_NO_SANDBOX`, and add security-audit checks for stale/missing sandbox browser Docker hash labels. Thanks @TerminalsandCoffee and @vincentkoc.
...
- Config/State: removed legacy `.Ironcliw` auto-detection/migration and `Ironcliw.json` config candidates. If you still have state/config under `~/.Ironcliw`, move it to `~/.Ironcliw` (recommended) or set `IRONCLIW_STATE_DIR` / `IRONCLIW_CONFIG_PATH` explicitly.
...
- Signal: render mention placeholders as `@uuid`/`@phone` so mention gating and Ironcliw targeting work. (#2013) Thanks @alexgleason.
...
- Skills: update session-logs paths from .Ironcliw to .Ironcliw. (#4502)
...
- Doctor: honor IRONCLIW_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- Discovery: shorten Bonjour DNS-SD service type to `_Ironcliw-gw._tcp` and update discovery clients/docs.
...
- Daemon: share profile/state-dir resolution across service helpers and honor `IRONCLIW_STATE_DIR` for Windows task scripts.
...
- Onboarding/Auth: honor `IRONCLIW_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `IRONCLIW_LAUNCHD_LABEL` / `IRONCLIW_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `Ironcliw doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Installer UX: support `IRONCLIW_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Config: expand `~` in `IRONCLIW_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
- CLI: respect `IRONCLIW_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`IRONCLIW_LIVE_GATEWAY_IMAGE_PROBE=1`).
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle IroncliwKit resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
